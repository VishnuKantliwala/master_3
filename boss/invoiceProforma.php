...
$sqlBooking = $cn->selectdb("SELECT i.entry_date,c.*,sc.* FROM tbl_proforma i,tbl_shipper c,tbl_service_confirmation sc WHERE i.service_confirmation_id = sc.service_confirmation_id AND sc.shipper_id = c.shipper_id AND i.proforma_id =".$invoice_id);
    ...
    <table width="100%" style="border:2px solid black;">
	 ...
	 ?>
                                    ...
                                    <td width="35%" style="border-top:0px" align="right">
                                        <p align="right" class="style6"><span style="color:#015289;">PROFORMA INVOICE NO:</span>
                                            <? echo $invoice_id;?>
                                        </p>
	 ...
	 ?>
		...
		$sql = $cn->selectdb("SELECT p.name,p.code,p.gst_rate,si.duration,si.yorm,si.quantity,si.service_rate,si.gst_charge,si.net_amount,si.total_amount,si.short_desc FROM tbl_product p,tbl_service_inclusion si WHERE si.product_id = p.product_id AND si.proforma_id = ".$invoice_id);
